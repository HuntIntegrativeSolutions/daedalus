...
- **Async:** `anyio` (runs on asyncio + trio). Async-first core; the **sync API is a generated shim** over anyio's blocking portal on a persistent background loop — never a second protocol stack.
