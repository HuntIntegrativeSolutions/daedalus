"""CIP status and vendor lookup tables.
...
EXTERNAL_ACCESS: Final[dict[int, str]] = {
    0: "Read/Write",
    1: "Reserved",
    2: "Read Only",
    3: "None",
...
KEYSWITCH: Final[dict[int, dict[int, str]]] = {
...
STATUS: Final[dict[int, str]] = {
    0x0: "Success",
    0x1: "The sender issued an invalid or unsupported encapsulation command",
    0x2: "Insufficient memory",
    0x3: "Poorly formed or incorrect data in the data portion",
    0x64: (
...
SERVICE_STATUS: Final[dict[int, str]] = {
    0x1: "Connection failure (see extended status)",
    0x2: "Insufficient resource",
    0x3: "Invalid value",
    0x4: (
        "IOI syntax error. A syntax error was detected decoding the Request Path"
        " (see extended status)"
    ),
    0x5: (
        "Destination unknown, class unsupported, instance undefined or structure element undefined"
        " (see extended status)"
    ),
    0x6: "Insufficient Packet Space",
    0x7: "Connection lost",
    0x8: "Service not supported",
    0x9: "Error in data segment or invalid attribute value",
    0xA: "Attribute list error",
    0xB: "State already exist",
    0xC: "Object state conflict",
    0xD: "Object already exist",
...
EXTEND_CODES: Final[dict[int, dict[int, str]]] = {
    0x1: {
        0x100: "Connection in use",
        0x103: "Transport not supported",
        0x106: "Ownership conflict",
        0x107: "Connection not found",
        0x108: "Invalid connection type",
        0x109: "Invalid connection size",
        0x110: "Module not configured",
        0x111: "EPR not supported",
        0x114: "Wrong module",
        0x115: "Wrong device type",
        0x116: "Wrong revision",
        0x118: "Invalid configuration format",
        0x11A: "Application out of connections",
        0x203: "Connection timeout",
        0x204: "Unconnected message timeout",
        0x205: "Unconnected send parameter error",
        0x206: "Message too large",
        0x301: "No buffer memory",
        0x302: "Bandwidth not available",
        0x303: "No screeners available",
        0x305: "Signature match",
        0x311: "Port not available",
        0x312: "Link address not available",
        0x315: "Invalid segment type",
        0x317: "Connection not scheduled",
    },
    0x4: {
        0x0: "Extended status out of memory",
        0x1: "Extended status out of instances",
    },
    0x5: {
        0x0: "Extended status out of memory",
        0x1: "Extended status out of instances",
...
VENDOR_IDS: Final[dict[str, int]] = {v: k for k, v in VENDORS.items()}


PRODUCT_TYPES: Final[dict[int, str]] = {
    0x0: "Generic Device (deprecated)",
    0x2: "AC Drive",
    0x3: "Motor Overload",
    0x4: "Limit Switch",
    0x5: "Inductive Proximity Switch",
    0x6: "Photoelectric Sensor",
    0x7: "General Purpose Discrete I/O",
    0x9: "Resolver",
    0xC: "Communications Adapter",
    0xE: "Programmable Logic Controller",
...
def decode_status(general: int, extended: int = 0) -> str:
    """Return human-readable description for a CIP service status code."""
    extended_map = EXTEND_CODES.get(general, {})
    if extended and extended in extended_map:
        return f"0x{general:02X}/0x{extended:04X}: {extended_map[extended]}"
    base = SERVICE_STATUS.get(general, f"Unknown status 0x{general:02X}")
    return f"0x{general:02X}: {base}"


def get_vendor(vendor_id: int) -> str:
    """Return vendor name for a given vendor ID, or a hex fallback."""
    return VENDORS.get(vendor_id, f"Unknown vendor 0x{vendor_id:04X}")
